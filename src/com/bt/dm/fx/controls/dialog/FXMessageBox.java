...
class FXMessageDialog extends Dialog<ButtonType> {
	...
	private boolean messageEnglishFont;
	private FXTextCmp errorMessageText;
	...
	public static FXMessageDialog getInstance() {
		if (instance == null) {
			...
			instance.initErrorText();
			instance.setAlertIcon(AlertType.INFORMATION);
			instance.initErrorPopup();
		}
	...
	private void initErrorText() {
		errorMessageText = new FXTextCmp(
				new FXTextCmpBuilder().wrappingWidth(SizeHelper.SCREEN_SIZE.getWidth() * 0.4));
		errorMessageText.textProperty().bind(this.message);
	}

	private void toggleEnglishFont() {
		if (this.messageEnglishFont) {
			errorMessageText.getStyleClass().add("english-font");
		} else {
			errorMessageText.getStyleClass().remove("english-font");
		}
	}
	...
	private void initErrorPopup() {
		...
		messagePanel.setCenter(errorMessageText);
	...
	private AlertIconDetails getIconStyles(AlertType alertType) {
		...
		errorMessageText.getStyleClass().remove("kannada-error-red-font");
		...
		case ERROR:
			...
			errorMessageText.getStyleClass().add("kannada-error-red-font");
			break;
		case WARNING:
